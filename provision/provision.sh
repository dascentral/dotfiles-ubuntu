...
# Drop a catch-all default server that rejects requests with unknown or
# missing Host/SNI headers. Without this, Nginx serves the first
# alphabetical site to scanners hitting the IP directly.
...
# Remove the packaged default site so it doesn't compete for default_server.
